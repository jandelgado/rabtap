...
func verifyMessagesOnTap(t *testing.T, consumer string, numExpected int,
	...
	tap := NewAmqpTap(testhelper.IntegrationURIFromEnv(), &tls.Config{}, log.New(os.Stderr, "", log.LstdFlags))
...
func TestIntegrationHeadersExchange(t *testing.T) {
	...
	conn, ch := testhelper.IntegrationTestConnection(t, "headers-exchange", "headers", 2, true)
	defer conn.Close()
	...
	testhelper.PublishTestMessages(t, ch, MessagesPerTest, "headers-exchange", "", amqp.Table{"header1": "test0"})
	...
	testhelper.VerifyTestMessageOnQueue(t, ch, "consumer2", MessagesPerTest, "queue-0", finishChan)
	requireIntFromChan(t, finishChan, MessagesPerTest)
}
...
func TestIntegrationDirectExchange(t *testing.T) {
	...
	testhelper.PublishTestMessages(t, ch, MessagesPerTest, "direct-exchange", "queue-0", nil)
...
func TestIntegrationTopicExchangeTapSingleQueue(t *testing.T) {
	...
	// establish sending exchange
	conn, ch := testhelper.IntegrationTestConnection(t, "topic-exchange", "topic", 2, false)
	defer conn.Close()
	...
	// inject messages into exchange. Each message should become visible
	// in the tap-exchange defined above.
	testhelper.PublishTestMessages(t, ch, MessagesPerTest, "topic-exchange", "queue-0", nil)
	testhelper.PublishTestMessages(t, ch, MessagesPerTest, "topic-exchange", "queue-1", nil)
	...
	testhelper.VerifyTestMessageOnQueue(t, ch, "consumer3", MessagesPerTest, "queue-1", finishChan)
	requireIntFromChan(t, finishChan, MessagesPerTest)
}
...
func TestIntegrationCloseTap(t *testing.T) {
	...
	conn, _ := testhelper.IntegrationTestConnection(t, "topic-exchange", "topic", 2, false)
	defer conn.Close()
